...
class StateMachine {
    ...
    init() {
        ...
        let alone = StateMachineSymbol(phrase: "There's so much you'll loose being alone.", options: [], sceneName: "alone", numberOfFrames: 1, timePerFrame: 0.1, pixelColor: .red, pixelPosition: CGPoint(x: 0, y: 0))
        let aloneAnswer = StateMachineSymbol(phrase: "You wander around, meaningless...", options: [], sceneName: "aloneAnswer", numberOfFrames: 1, timePerFrame: 0.1, pixelColor: .appWhite, pixelPosition: CGPoint(x: 0, y: 0))
        ...
        let human = StateMachineSymbol(phrase: "You're part of a conscious been.", options: [], sceneName: "human", numberOfFrames: 3, timePerFrame: 0.1, pixelColor: .red, pixelPosition: CGPoint(x: 0, y: 0))
        let dream = StateMachineSymbol(phrase: "", options: [], sceneName: "dream", numberOfFrames: 3, timePerFrame: 0.1, pixelColor: .red, pixelPosition: CGPoint(x: 0, y: 0))
        let dreamAnswer = StateMachineSymbol(phrase: "Of course this means something. It means everything, and nothing. It means whatever you gave it a meaning. You mean whatever you give yourself a meaning.", options: [], sceneName: "dreamAnswer", numberOfFrames: 1, timePerFrame: 0.1, pixelColor: .appWhite, pixelPosition: CGPoint(x: 0, y: 0))
        ...
        let nightmareAnswer = StateMachineSymbol(phrase: "Every single thing in the universe is part of it. Beginnings are just a part of the end. Ends are just a part of the beginning.", options: [], sceneName: "nightmareAnswer", numberOfFrames: 1, timePerFrame: 0.1, pixelColor: .appOrange3, pixelPosition: CGPoint(x: 0, y: 0))
